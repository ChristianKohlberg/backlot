...
backlot puts a working instance of a web application in front of a coding agent or
...
3. **Never be CI** — CI may call backlot, never the reverse. CI proves committed state
   to a team; backlot proves arbitrary (usually dirty) state to one consumer.
